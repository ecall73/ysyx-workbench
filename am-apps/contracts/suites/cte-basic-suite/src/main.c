...
static void point_event_dispatch(void) { basic_phase = 1; int before = event_count; yield(); CONTRACT_CHECK_POINT("event-dispatch", event_count == before + 1, "yield-count"); basic_phase = 2; before = event_count; do_syscall_ecall(); CONTRACT_CHECK_POINT("event-dispatch", event_count == before + 1, "syscall-count"); }
static void point_mepc_return(void) { basic_phase = 1; int before = event_count; yield(); CONTRACT_CHECK_POINT("mepc-return", event_count == before + 1, "yield-return"); basic_phase = 2; before = event_count; do_syscall_ecall(); CONTRACT_CHECK_POINT("mepc-return", event_count == before + 1, "syscall-return"); }
static void point_interrupt_control(void) { iset(false); CONTRACT_CHECK_POINT("interrupt-control", !ienabled(), "disabled"); basic_phase = 1; yield(); iset(true); CONTRACT_CHECK_POINT("interrupt-control", !ienabled(), "enabled-current-semantics"); yield(); iset(false); CONTRACT_CHECK_POINT("interrupt-control", !ienabled(), "disabled-again"); }
