...
enum riscv_difftest_profile_id {
  ...
  RISCV_DIFFTEST_PROFILE_RV32E_NPC_NEMU = 2,
};
...
#define RISCV_DIFFTEST_RV32E_FEATURES \
  (RISCV_DIFFTEST_ISA_E | RISCV_DIFFTEST_ISA_ZICSR | \
   RISCV_DIFFTEST_ISA_ZIFENCEI)

#define RISCV_DIFFTEST_RV32E_REQUIRED_CAPS \
  (RISCV_DIFFTEST_CAP_ARCH_STEP | RISCV_DIFFTEST_CAP_TYPED_SKIP | \
   RISCV_DIFFTEST_CAP_PURE_OBSERVATION | RISCV_DIFFTEST_CAP_SYNC_STATE)
...
#define RISCV_DIFFTEST_RV32E_STATE_FIELDS \
  (RISCV_DIFFTEST_FIELD_PC | RISCV_DIFFTEST_FIELD_GPR | \
   RISCV_DIFFTEST_FIELD_FPR | RISCV_DIFFTEST_FIELD_FCSR | \
   RISCV_DIFFTEST_FIELD_PRIV | RISCV_DIFFTEST_FIELD_MSTATUS | \
   RISCV_DIFFTEST_FIELD_MTVEC | RISCV_DIFFTEST_FIELD_MEPC | \
   RISCV_DIFFTEST_FIELD_MCAUSE | RISCV_DIFFTEST_FIELD_SATP)

#define RISCV_DIFFTEST_RV32E_GPR_MASK UINT32_C(0x0000ffff)
...
static inline uint64_t riscv_difftest_skip_sync_fields(uint32_t profile_id,
  ...
  switch (profile_id) {
    case RISCV_DIFFTEST_PROFILE_RV32GC_NEMU_SPIKE:
      ...
      break;
    case RISCV_DIFFTEST_PROFILE_RV32E_NPC_NEMU:
  ...
  switch (reason) {
    ...
    case RISCV_DIFFTEST_SKIP_PROFILE_OWNED_STATIC:
      return profile_id == RISCV_DIFFTEST_PROFILE_RV32GC_NEMU_SPIKE
          ? base_fields : 0;
    default:
      return 0;
  }
