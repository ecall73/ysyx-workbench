...
static inline bool in_soc_boot_range(paddr_t addr, int len) {
  return in_flash_range(addr, len) || in_sram_range(addr, len);
}
...
static void out_of_bound(paddr_t addr) {
  panic("address = " FMT_PADDR " is out of bound. valid ranges: pmem[" FMT_PADDR ", " FMT_PADDR "], "
      ...
      addr, PMEM_LEFT, PMEM_RIGHT, FLASH_BASE, FLASH_RIGHT, SRAM_BASE, SRAM_RIGHT, SDRAM_BASE, SDRAM_RIGHT, cpu.pc);
}
