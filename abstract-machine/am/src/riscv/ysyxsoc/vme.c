...
static Area segments[] = {
  RANGE(CLINT_BASE, CLINT_BASE + 0x10000u),
  RANGE(SRAM_BASE, SRAM_BASE + 0x2000u),
  RANGE(UART_BASE, GPIO_BASE + 0x1000u),
  RANGE(PS2_BASE, PS2_BASE + 0x1000u),
  RANGE(VGA_BASE, VGA_END + 1u),
  RANGE(SDRAM_BASE, SDRAM_BASE + 0x02000000u),
};
...
static inline void flush_tlb_page(void *va) {
...
static inline void set_satp(void *pdir) {
  ...
  asm volatile("csrw satp, %0" : :
      "r"(mode | ((uintptr_t)pdir >> 12)) : "memory");
  flush_tlb();
}
