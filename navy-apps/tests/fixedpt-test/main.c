...
int main() {
  ...
  assert(fixedpt_divi(fixedpt_rconst(-3.5), 2) == fixedpt_rconst(-1.75));
  ...
  assert(fixedpt_floor(-fixedpt_fromint(2)) == -fixedpt_fromint(2));
  assert(fixedpt_ceil(-fixedpt_fromint(2)) == -fixedpt_fromint(2));

  printf("fixedpt-test: PASS\n");
  return 0;
}
